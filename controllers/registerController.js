...
const createUser = async (req, res) => {
  ...
  const passwordMessage = passwordValidation(password, confirmPassword);

  if (passwordMessage || regexMessage) {
    return res.render('register', { message: passwordMessage || regexMessage });
  }

  const isInserted = await insertUser(email, password, name, lastName);

  if (!isInserted) {
    return res.render('register', { message: 'erro ao inserir usuario no banco de dados' });
  }
